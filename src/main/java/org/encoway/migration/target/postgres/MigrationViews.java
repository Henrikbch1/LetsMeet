...
class MigrationViews {
    ...
    private static final String VIEW_MIGRATION_REJECTIONS = "migration_rejections";
    ...
    private static final String CREATE_MIGRATION_USER_HOBBIES_VIEW = """
            ...
            SELECT
                ...
                hobby.source::text AS source
            FROM hobby
            JOIN person ON person.person_id = hobby.user_id
            """.formatted(VIEW_MIGRATION_USER_HOBBIES);
    ...
    // Documents every physical import record that could not be taken over.
    private static final String CREATE_MIGRATION_REJECTIONS_VIEW = """
            CREATE VIEW %s AS
            SELECT
                migration_rejection.source::text AS source,
                migration_rejection.source_ref::text AS source_ref,
                migration_rejection.reason::text AS reason
            FROM migration_rejection
            """.formatted(VIEW_MIGRATION_REJECTIONS);

    protected void dropMigrationViews(Connection connection) throws SQLException {
        try (Statement statement = connection.createStatement()) {
            dropMigrationViews(statement);
        }
    }

    protected void createMigrationViews(Connection connection) throws SQLException {
        try (Statement statement = connection.createStatement()) {
            dropMigrationViews(statement);
            ...
            statement.executeUpdate(CREATE_MIGRATION_REJECTIONS_VIEW);
        }
    }

    private void dropMigrationViews(Statement statement) throws SQLException {
        // Legacy V2 views: no longer (re-)created, but still dropped so a reset also cleans up a
        // database that still has them from a previous run.
        ...
        statement.executeUpdate("DROP VIEW IF EXISTS %s".formatted(VIEW_MIGRATION_REJECTIONS));
